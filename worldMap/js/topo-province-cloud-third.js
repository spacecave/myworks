(function(win){
    ...
    win.getProvinceCloudTopoThird = function(divid, name, webset, endname){
        var proTopo = this;
        var _topo = Graph.init(document.getElementById(divid), {
            cx: 0.65,
            saveposflag: "position_cloud_third_" + name
        });
            ...
            var drawBezier = function(data) {
                ...
                if(devI) {
                    ...
                    for(var i in devI) {
                        ...
                        var areax = ibox.x - 50;
                        var areay = ibox.y - 50;
                        var areaWidth = ibox.width + 100;
                        var areaHeight = ibox.height + 100;
                        pp.rect(areax, areay, areaWidth, areaHeight, 10).attr({
                            stroke: "#fff",
                            "stroke-width": 1,
                            "stroke-dasharray": ["- "]
                        });
                        pp.text(areax + areaWidth / 2, areay + 20, i).attr({
                            fill: "#fff",
                            "font-size": "20px",
                            "font-family": "微软雅黑"
                        });
        ...
        proTopo.loadTopo = function(obj) {
            _topo.showLoading();
            $("#pro-detail-font-3").html("全球--" + name + "--" + endname);
